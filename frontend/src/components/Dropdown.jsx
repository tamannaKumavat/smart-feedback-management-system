...
export default function Dropdown({
  ...
  const baseBtn = [
    "flex w-full cursor-pointer items-center justify-between gap-3 rounded-2xl border border-[#ECECF4] bg-white",
    "py-3 pl-4 pr-3 text-left text-[13px] font-medium transition",
  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      ...
      >
        <span
          className={`min-w-0 flex-1 truncate ${isPlaceholder ? "font-normal text-[#D1D1E0]" : "text-[#555060]"}`}
        >
          {displayLabel}
        </span>
        <FiChevronDown
          className={`h-5 w-5 shrink-0 text-[#A880FF] transition-transform duration-200 ${open ? "rotate-180" : ""}`}
          strokeWidth={2.25}
          aria-hidden
        />
      </button>
      {open ? (
        <ul
          ...
          className={`absolute left-0 right-0 z-50 mt-2 max-h-60 overflow-auto rounded-2xl border border-[#ECECF4] bg-white py-2 ${menuShadow}`}
        >
          {options.map((opt) => {
            ...
            return (
              <li
                ...
                className={`mx-1.5 cursor-pointer rounded-xl px-4 py-3 text-[13px] font-medium transition-colors sm:mx-2 sm:text-[14px] ${
                  selectedOpt
                    ? "bg-[#A880FF] text-white"
                    : "text-[#555060] hover:bg-[#A880FF] hover:text-white"
                }`}
              >
                {opt.label}
              </li>
