...
export default function TicketHistory({
      ...
      const convo = Array.isArray(t.conversation)
        ? t.conversation
            .map((c) => {
              ...
              if (c.confirmationRequest?.summary) bits.push(c.confirmationRequest.summary);
              return bits.join(" ");
            })
            .join(" ")
        : "";
      const hay = `${t.ticketRef ?? ""} ${t.name ?? ""} ${t.subject ?? ""} ${t.category ?? ""} ${t.preview ?? ""} ${t.snippet ?? ""} ${convo}`.toLowerCase();
  ...
  const messages = useMemo(() => {
    ...
    let list = legacy?.length ? legacy : getTicketHistoryMessagesForThread(active);
    const hasInteractiveConfirmation = active.conversation?.some((turn) => turn.confirmationRequest?.summary);
    ...
    <div className="flex h-full min-h-0 min-w-0 w-full flex-col overflow-hidden bg-white md:flex-row">
      ...
      >
        <div className={`flex shrink-0 items-center justify-between gap-2 border-b ${hairline} px-4 py-3`}>
          <h2 className="text-[15px] font-semibold text-slate-900">Ticket History</h2>
          <button
            type="button"
        ...
        <div className={`min-h-0 flex-1 overflow-y-auto overscroll-contain ${scrollPretty}`}>
          {filteredThreads.length === 0 ? (
            <p className="px-4 py-8 text-center text-[13px] text-slate-500">No threads match your search.</p>
          ) : (
                  ...
                  <li key={t.id}>
                    <button
                      ...
                      className={`flex w-full gap-3 px-3 py-3 text-left transition ${
                        selected ? "bg-slate-50 shadow-[inset_3px_0_0_0_#0d9488]" : "hover:bg-slate-50"
                      }`}
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="shrink-0 text-[11px] text-slate-400">{t.timeAgo}</span>
                        </div>
                        <p className="truncate text-[12px] font-medium text-slate-600">{t.subject}</p>
                        <p className="mt-0.5 line-clamp-2 text-[11px] leading-snug text-slate-500">{t.snippet}</p>
      ...
      <section className="flex min-w-0 flex-1 flex-col bg-white">
        {active ? (
          <>
            <header className={`flex shrink-0 items-center justify-between gap-3 border-b ${hairline} px-4 py-3 sm:px-5`}>
            ...
            >
              <div className="mx-auto max-w-[720px] space-y-4">
                {messages.map((msg) => {
                  ...
                  const bubbleBase = "rounded-2xl px-4 py-2.5 text-[13px] leading-relaxed shadow-sm";
                  const bubble =
                    outgoing
                          ...
                          {isConfirmation ? (
                            <>
                              ...
                              <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                                ...
                                <p className="mt-1.5 text-[13px] leading-relaxed text-slate-800">{msg.summary}</p>
                              </div>
                              ...
                              {showConfirmActions ? (
                                <div className="mt-3 flex flex-wrap gap-2">
                                  <button
                                    ...
                                    onClick={() =>
                                      setConfirmationChoice((c) => ({ ...c, [active.id]: "no" }))
                                    }
                                  >
                                    No
                                  </button>
                                  <button
                                    ...
                                    onClick={() =>
                                      setConfirmationChoice((c) => ({ ...c, [active.id]: "yes" }))
                                    }
                                  >
                                    Yes
                                  </button>
